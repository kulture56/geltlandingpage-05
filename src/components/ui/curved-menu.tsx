...
const CustomFooter: React.FC = () => {
  return <div className="flex w-full text-sm justify-between text-black px-10 md:px-24 py-5">
  ...
  return <motion.div onClick={handleClick} initial="initial" whileHover="whileHover" className="group relative flex items-center justify-between border-b border-black/30 py-4 transition-colors duration-500 md:py-8 uppercase">
			<a ref={ref} onMouseMove={handleMouseMove} href={href}>
				<div className="relative flex items-start bg-slate-50">
					<span className="transition-colors duration-500 mr-2 text-purple-800 font-extrabold text-lg">
						{index}.
					</span>
...
const Curve: React.FC = () => {
  ...
  return <svg className="absolute top-0 -left-[99px] w-[100px] stroke-none h-full" style={{
    fill: "#ffffff"
  }}>
...
const Header: React.FC<iHeaderProps> = ({
  ...
  return <>
			<div className="relative">
				<div onClick={handleClick} className="fixed -right-1 top-0 md:-right-1 m-5 z-50 w-12 h-12 rounded-none flex items-center justify-center cursor-pointer bg-white">
